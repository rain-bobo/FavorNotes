...
import com.example.favornotes.adapter.ChartVPAdapter;
import com.example.favornotes.db.DBManager;
import com.example.favornotes.frag_chart.IncomChartFragment;
import com.example.favornotes.frag_chart.OutcomChartFragment;
import com.example.favornotes.utils.CalendarDialog;
...
//        将Fragment加载到Acitivy当中
    ...
    private void showCalendarDialog() {
        ...
        dialog.setOnRefreshListener(new CalendarDialog.OnRefreshListener() {
            ...
            public void onRefresh(int selPos, int year, int month) {
                StatsActivity.this.selectPos = selPos;
                StatsActivity.this.selectMonth = month;
                initStatistics(year,month);
    ...
    private void setButtonStyle(int kind){
        if (kind == 0) {
            outBtn.setBackgroundResource(R.drawable.recordbtn_bg);
            outBtn.setTextColor(Color.WHITE);
            inBtn.setBackgroundResource(R.drawable.dialog_btn_bg);
            inBtn.setTextColor(Color.BLACK);
        }else if (kind == 1){
            inBtn.setBackgroundResource(R.drawable.recordbtn_bg);
