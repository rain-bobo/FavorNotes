...
import com.example.favornotes.utils.FloatUtils;
...
//        循环读取游标内容，存储到对象当中
     ...
     * 根据事由搜索随礼或者收礼的情况列表
     * */
    public static List<AccountBean>getAccountListByRemarkFromAccounttb(String reason){
        List<AccountBean>list = new ArrayList<>();
        String sql = "select * from accounttb where reason like '%"+reason+"%'";
        Cursor cursor = db.rawQuery(sql, null);
        while (cursor.moveToNext()) {
            int id = cursor.getInt(cursor.getColumnIndex("id"));
            String typename = cursor.getString(cursor.getColumnIndex("typename"));
            String name = cursor.getString(cursor.getColumnIndex("name"));
            String thereason = cursor.getString(cursor.getColumnIndex("reason"));
            String time = cursor.getString(cursor.getColumnIndex("time"));
            int sImageId = cursor.getInt(cursor.getColumnIndex("sImageId"));
            int kind = cursor.getInt(cursor.getColumnIndex("kind"));
            float money = cursor.getFloat(cursor.getColumnIndex("money"));
            ...
            int day = cursor.getInt(cursor.getColumnIndex("day"));
            AccountBean accountBean = new AccountBean(id, typename, sImageId, name, thereason, money, time, year, month, day, kind);
            list.add(accountBean);
        }
        return list;
    }
     ...
     * 获取记账表当中某一月的支出或者收入情况
     * */
    public static List<AccountBean>getOneAccountListOneMonthFromAccounttb(int year, int month,int kind){
        List<AccountBean>list = new ArrayList<>();
